...
describe("armado del batch", () => {
  it("replica el formato observado en el editor para el período nuevo", () => {
  ...
  it("el batch completo lleva los períodos actuales más el nuevo, nunca solo el nuevo", () => {
    const internos = [
      { id: "p1", name: "A", start_time_ms: 10, end_time_ms: 20, athletes: [{ id: "x" }] },
      { id: "p2", name: "B", start_time_ms: 30, end_time_ms: 40, athletes: [] },
      { name: "sin id" },
    ];
    const connect = [{ id: "p1", period_depth_id: "depth-1" }, { id: "p2", period_depth_id: null }];
    const nuevo = armarPeriodoNuevo({ id: "n", nombre: "TEST APP 01", startMs: 50, endMs: 60, athletes: [] });

    const batch = armarBatchCompleto({ periodosInternos: internos, periodosConnect: connect, nuevo });

    expect(Object.keys(batch)).toEqual(["periods"]);
    expect(batch.periods).toHaveLength(3);
    expect(batch.periods[0]).toEqual({
      ...
      end_time_ms: 20,
      athletes: [{ id: "x" }],
      period_depth_id: "depth-1",
    });
    expect(batch.periods[1].period_depth_id).toBeUndefined();
    expect(batch.periods[2]).toBe(nuevo);
  });

  it("sin período nuevo devuelve solo los actuales (sirve para restaurar)", () => {
    const batch = armarBatchCompleto({
      periodosInternos: [{ id: "p1", name: "A", start_time_ms: 1, end_time_ms: 2, athletes: [] }],
    });
    expect(batch.periods).toHaveLength(1);
    expect(batch.periods[0].id).toBe("p1");
  });
});
