...
export const rutasSonda = ({ activityId, periodId }) => {
  ...
  // Data Injection: PUT /injection/activities/{id} es la única operación
  // documentada que recibe periods. OPTIONS revela si el router registra la
  // ruta; el GET confirma que /injection existe. Ninguno prueba por sí solo
  // que el PUT acepte una actividad real: eso lo decide el handler, no el
  // router. Sigue siendo GET/OPTIONS: no se envía el PUT.
  const inyeccion = `/injection/activities/${encodeURIComponent(activityId)}`;
  rutas.push(
    { clave: "inyeccion-get", metodo: "GET", ruta: inyeccion },
    { clave: "inyeccion-options", metodo: "OPTIONS", ruta: inyeccion },
  );
...
// OpenField manda sus booleanos como 0/1 (a veces "0"/"1" o true/false).
export const marcaBooleana = (valor) => {
  if (valor === true || valor === 1 || valor === "1") return true;
  if (valor === false || valor === 0 || valor === "0") return false;
  return null;
};

const textoCorto = (valor) =>
  typeof valor === "string" || typeof valor === "number" ? String(valor).slice(0, 60) : null;
...
export const describirCuerpo = (texto, { maximoSnippet = 300 } = {}) => {
  ...
  if (json && typeof json === "object") {
    ...
    return {
      ...
      // Marcas de la actividad que deciden qué vía de escritura aplica.
      ...(json.is_injected !== undefined ? { is_injected: marcaBooleana(json.is_injected) } : {}),
      ...(textoCorto(json.process_status) !== null
        ? { process_status: textoCorto(json.process_status) }
        : {}),
      ...(textoCorto(json.bake_status) !== null ? { bake_status: textoCorto(json.bake_status) } : {}),
      ...
      ...(primero
        ? {
            muestraPeriodo: {
              ...
              // El servicio interno usa milisegundos en vez de segundos + centésimas.
              ...(primero.start_time_ms !== undefined
                ? { start_time_ms: primero.start_time_ms ?? null }
                : {}),
              ...(primero.end_time_ms !== undefined ? { end_time_ms: primero.end_time_ms ?? null } : {}),
            },
          }
        : {}),
