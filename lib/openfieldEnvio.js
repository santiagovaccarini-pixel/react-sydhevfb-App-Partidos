...
export const planificarCortes = ({
  ...
  (Array.isArray(tareas) ? tareas : []).forEach((tarea) => {
    ...
    // El inicio y el fin que OpenField informa para la actividad no siempre
    // abarcan sus períodos: en 26-05 T la actividad "dura" 7 minutos y tiene
    // períodos 20 minutos después, cargados desde el editor. Como el editor
    // los acepta, acá solo se avisa; no se rechaza.
    if (ventanaConocida && (preparada.inicioMs < inicioActividad || preparada.finMs > finActividad)) {
      avisos.push({
        tareaId: preparada.id,
        nombre: preparada.nombre,
        aviso: "Queda fuera del horario que OpenField informa para la actividad.",
      });
    }
  ...
  if (errores.length > 0) {
    return { ok: false, errores, avisos, tareas: planTareas };
  }
