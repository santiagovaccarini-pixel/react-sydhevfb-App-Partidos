...
// con lo que recibe. Comprobado el 2026-09-18: mandar solo el período nuevo
// borró los 8 originales de 26-05 T. Por eso siempre se manda el set completo:
// los períodos actuales (tal cual vinieron del servicio interno) más el nuevo.
// period_depth_id no viene en la lectura interna; se toma del snapshot de
// Connect por id para no perderlo.
export const armarBatchCompleto = ({ periodosInternos, periodosConnect = [], nuevo = null }) => {
  ...
  const actuales = (Array.isArray(periodosInternos) ? periodosInternos : [])
    .filter((periodo) => periodo?.id)
    .map((periodo) => {
      const depth = depthPorId.get(String(periodo.id));
      return {
        id: String(periodo.id),
        name: String(periodo.name || ""),
  ...
  return { periods: nuevo ? [...actuales, nuevo] : actuales };
};
