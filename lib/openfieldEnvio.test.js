...
describe("planificarCortes", () => {
  it("convierte las tareas en períodos, reutiliza ids y preserva lo ajeno", () => {
    ...
    expect(plan.batch.periods[2]).toMatchObject({ name: "2. POSSE", start_time_ms: T0 + 25 * MIN, athletes: [{ id: "a2" }] });
  ...
  it("rechaza tareas inválidas o fuera del horario de la actividad, sin armar batch", () => {
    const plan = planificarCortes({
      tareas: [
        { id: "t1", nombre: "", inicio: T0, fin: T0 + MIN, participantes: [] },
        { id: "t2", nombre: "Tarde", inicio: T0 + 80 * MIN, fin: T0 + 100 * MIN, participantes: [] },
    ...
    expect(plan.errores.map((e) => e.tareaId)).toEqual(["t1", "t2"]);
    expect(plan.errores[0].error).toMatch(/nombre/);
    expect(plan.errores[1].error).toMatch(/fuera del horario/);
