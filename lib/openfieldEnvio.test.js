...
describe("planificarCortes", () => {
  ...
  // Comprobado en 26-05 T: un corte que termina después del fin de los datos
  // vuelve con 422; el mismo corte terminando antes se graba y se grafica.
  it("una tarea fuera del rango de datos de la actividad se rechaza con la ventana", () => {
    const plan = planificarCortes({
      tareas: [
        { id: "t2", nombre: "Tarde", inicio: T0 + 80 * MIN, fin: T0 + 100 * MIN, participantes: [{ atletaId: "a1" }] },
        { id: "t0", nombre: "Temprano", inicio: T0 - MIN, fin: T0 + MIN, participantes: [{ atletaId: "a1" }] },
    ...
    expect(plan.errores).toEqual([
      { tareaId: "t2", nombre: "Tarde", error: "Termina después del fin de los datos de la sesión en OpenField.", ventana: { inicioMs: T0, finMs: T0 + 90 * MIN } },
      { tareaId: "t0", nombre: "Temprano", error: "Empieza antes del inicio de los datos de la sesión en OpenField.", ventana: { inicioMs: T0, finMs: T0 + 90 * MIN } },
    ]);
    expect(plan.batch).toBeUndefined();
  });

  it("sin un fin válido de la actividad no controla el rango", () => {
    const plan = planificarCortes({
    ...
    expect(plan.ok).toBe(true);
    expect(plan.errores).toBeUndefined();
  });
