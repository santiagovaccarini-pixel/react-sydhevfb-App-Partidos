...
describe("planificarCortes", () => {
  ...
  it("rechaza participantes que no tienen datos en la sesión, con sus ids", () => {
    const plan = planificarCortes({
      tareas: tareas(),
      actividadInterna: actividad(),
      generarId,
      atletasPermitidos: new Set(["a1"]),
    });
    expect(plan.ok).toBe(false);
    expect(plan.errores).toEqual([
      { tareaId: "t1", nombre: "2. POSSE", error: "1 participante(s) no tienen datos en esta sesión de OpenField.", atletasFuera: ["a2"] },
      { tareaId: "t2", nombre: "3. PJ RED", error: "1 participante(s) no tienen datos en esta sesión de OpenField.", atletasFuera: ["a2"] },
    ]);
  });
