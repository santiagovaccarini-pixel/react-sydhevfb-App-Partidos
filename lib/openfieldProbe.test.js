...
describe("marcaBooleana", () => {
  it("acepta booleanos, 0/1 y '0'/'1'; lo demás es desconocido", () => {
    expect(marcaBooleana(true)).toBe(true);
    expect(marcaBooleana(1)).toBe(true);
    expect(marcaBooleana("1")).toBe(true);
    expect(marcaBooleana(false)).toBe(false);
    expect(marcaBooleana(0)).toBe(false);
    expect(marcaBooleana("0")).toBe(false);
    expect(marcaBooleana(null)).toBeNull();
    expect(marcaBooleana("si")).toBeNull();
    expect(marcaBooleana(undefined)).toBeNull();
  });
});
...
describe("describirCuerpo", () => {
  ...
  it("lee is_injected aunque venga como 0/1 y expone los estados de proceso", () => {
    const real = describirCuerpo(
      JSON.stringify({ id: "x", is_injected: 0, process_status: "processed", bake_status: 2 }),
    );
    expect(real.is_injected).toBe(false);
    expect(real.process_status).toBe("processed");
    expect(real.bake_status).toBe("2");

    expect(describirCuerpo(JSON.stringify({ is_injected: "1" })).is_injected).toBe(true);
    expect(describirCuerpo(JSON.stringify({ is_injected: null })).is_injected).toBeNull();
    expect(describirCuerpo(JSON.stringify({ id: "x" })).is_injected).toBeUndefined();
  });
