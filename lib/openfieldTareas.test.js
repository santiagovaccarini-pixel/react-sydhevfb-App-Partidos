...
describe("participantes en el formato del servicio interno", () => {
  it("junta los atletas de todos los períodos de la actividad por athlete_id", () => {
    const mapa = atletasDeActividad({
      periods: [
        { id: "p1", athletes: [{ athlete_id: "a1", id: "fila-1", jersey: 7 }] },
        { id: "p2", athletes: [{ athlete_id: "a1", id: "fila-9" }, { athlete_id: "a2", id: "fila-2" }] },
      ],
    });
    expect([...mapa.keys()]).toEqual(["a1", "a2"]);
    expect(mapa.get("a1")).toEqual({ athlete_id: "a1", id: "fila-1", jersey: 7 });
  });

  it("copia el objeto interno si el atleta ya está en la actividad y manda { athlete_id } si no", () => {
    const atletas = atletasDeActividad({ periods: [{ id: "p1", athletes: [{ athlete_id: "a1", id: "fila-1" }] }] });
    const { periodos } = tareaAPeriodos(prepararTarea(base()), { generarId, atletas });
    expect(periodos[0].athletes).toEqual([{ athlete_id: "a1", id: "fila-1" }, { athlete_id: "a2" }]);
  });

  it("la verificación identifica por athlete_id aunque el objeto traiga otro id", () => {
    const esperados = [
      { id: "p1", tareaId: "t1", name: "X", start_time_ms: 1000, end_time_ms: 2000, athletes: [{ athlete_id: "a1", id: "fila-1" }, { athlete_id: "a2" }] },
    ];
    const snapshot = { periods: [{ id: "p1", name: "X", start_ms: 1000, end_ms: 2000, athletes: [{ id: "a2" }, { id: "a1" }] }] };
    expect(verificarPeriodos({ esperados, snapshot })[0].ok).toBe(true);
  });
});
...
describe("ventanasDeTarea y tareaAPeriodos", () => {
  it("solo totales: un único período de punta a punta con todos", () => {
    ...
    expect(periodos[0]).toMatchObject({
      ...
      athletes: [{ athlete_id: "a1" }, { athlete_id: "a2" }],
      tareaId: "t1",
  ...
  it("parciales: un período aparte con el mismo nombre por cada ventana distinta", () => {
    ...
    const todos = periodos.flatMap((p) => p.athletes.map((a) => a.athlete_id));
  ...
  it("reutiliza los ids asignados al regenerar y retira las ventanas que desaparecen", () => {
    ...
    expect(segunda.periodos[0].athletes).toEqual([{ athlete_id: "a1" }, { athlete_id: "a2" }]);
